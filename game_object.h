...
public:
    ...
    {
        auto lhs_center =
            Vec2{position.x + (width / 2.0), position.y + (height / 2.0)};
        auto rhs_center = Vec2{rhs.position.x + (rhs.width / 2.0),
                               rhs.position.y + (rhs.height / 2.0)};
        if (std::abs(lhs_center.x - rhs_center.x) >
            (width / 2.0) + (rhs.width / 2.0)) {
            return false;
        }
        if (std::abs(lhs_center.y - rhs_center.y) >
            (height / 2.0) + (rhs.height / 2.0)) {
            return false;
        }
        return true;
    }
    ...
    void set_to_destroy()
    {
        if (can_be_destroyed) { destroy = true; }
    }

    bool should_be_destroyed() { return destroy; }
